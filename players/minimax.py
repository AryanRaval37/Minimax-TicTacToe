...
class minimaxPlayer:
    ...
    def minimax(self, board, max = True):
        ...
        if max:
            ...
            for move in board.validMoves():
                ...
                curEval = self.minimax(board, max=False)
                board.remove(move)
        ...
        if not max:
            ...
            for move in board.validMoves():
                board.place(move, -self.player) # place that move
                # get evaluation of this move 
                curEval = self.minimax(board, max=True)
                board.remove(move)
                if (curEval[1] < bestScore):
                    bestScore = curEval[1]
                    bestMove = move
    ...
    def play(self, board):
        ...
        bestMove, bestScore = self.minimax(board)
        return bestMove
